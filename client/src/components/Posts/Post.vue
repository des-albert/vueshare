        ...
        <v-card hover>
          ...
          <v-tooltip right>
            <template v-slot:activator="{ on }">
              <v-img v-on="on" @click="toggleImageDialog" :src="getPost.imageUrl" id="post___image"> </v-img>
            </template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        messageBody: '',
        isFormValid: true,
        messageRules: [message => !!message || 'Message is required', message => message.length < 50 || 'Message must be less than 50 characters']
      };
    },
    apollo: {
      getPost: {
